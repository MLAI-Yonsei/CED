...
def get_raw_data(dataset, split='train', trigger_word=None, poison_ratio=0.1, label_transform=None,
    ...
    if dataset == '20news':
        ...
        with open(os.path.join(data_path, f'{ttt}/label.txt'), 'r') as f:
        ...
        with open(os.path.join(data_path, 'test/seq.in'), 'r') as f:
            raw_texts = [x.strip() for x in f.readlines()]
        with open(os.path.join(data_path, 'test/label.txt'), 'r') as f:
            raw_labels = [label2id[x.strip()] for x in f.readlines()]
...
class BertDataLoader2:
#with label info
    def __init__(self, texts,labels, split, tokenizer, batch_size, shuffle=False,
                 add_noise=False, noise_freq=0.1, label_noise_freq=0.0, few_shot_dir='./dataset/sst-2/16-shot/16-42', max_padding=None):
...
def data_loader(texts,labels, split, tokenizer, batch_size, shuffle=False, add_noise=False, noise_freq=0.1,\
     label_noise_freq=0.0, few_shot_dir='./dataset/sst-2/16-shot/16-42', max_padding=None):
    return BertDataLoader2(texts,labels, split, tokenizer, batch_size, shuffle, add_noise, noise_freq, label_noise_freq, few_shot_dir, max_padding)
